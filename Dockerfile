...
RUN \
      ...
      python3 -m pip install --no-cache-dir --break-system-packages --prefix=/usr/local pipx uv
...
RUN \
      curl -fsSL -o /usr/local/bin/antigravity.install.sh \
...
ENV PATH="/home/${USER_NAME}/.local/bin:/home/${USER_NAME}/.antigravity/bin:${PATH}"
